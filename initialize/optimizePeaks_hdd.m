function WUinit = optimizePeaks_hdd(ops, uprojsize)
...
ops.GPU = 1;
uBase = zeros(ops.nFiltMax, nProj, 'single');
nS    = zeros(ops.nFiltMax, 1, 'single');
ncurr = 1;

fid   = fopen(ops.uprojpath, 'r');

for ibatch = 1:nbatch
    
    [b, e] = range_idx(ibatch, nSpikesPerBatch, nspike);

    offset_batch = 4 * nSpikesPerBatch * (ibatch-1) * nProj;
    fseek(fid, offset_batch, 'bof');
    uS = fread(fid, [nProj e-b+1], '*single')';
...
for i = 1:10
    ...
    for ibatch = 1:nbatch
        ...
        offset_batch = 4 * nSpikesPerBatch * (ibatch-1) * nProj;
        fseek(fid, offset_batch, 'bof');
        uS = fread(fid, [nProj arr_rows], '*single')';
        
        
        if ops.GPU
            clips = reshape(gpuArray(uS), arr_rows, nProj);
        else
            clips = reshape(uS, arr_rows, nProj);
        end
        ci = clips * U;
