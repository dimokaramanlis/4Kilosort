function rez = fitTemplatesSplit(rez, chunckInds)
...
Nchan 	= numel(chunckInds); %ops.Nchan;
...
switch ops.initialize
    case 'fromData'
        uproj = get_uprojSplit(rez,chunckInds);
        WUinit    = optimizePeaksSplit(ops, uproj,chunckInds);%does a scaled kmeans 
        dWU    = WUinit(:,:, 1:Nfilt);
        %             dWU = alignWU(dWU);
